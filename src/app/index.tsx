...
/**
 * Phase 03 design-system gallery. Every foundational component is rendered here
 * so both themes can be checked in one pass. Phase 04 replaces this with the
 * real app shell.
 */
  ...
  return (
    <Screen scroll padded>
      <View className="pb-5 pt-4">
        <Text variant="display" className="text-[34px] leading-none">
          BANDMATE
        </Text>
      ...
      <Section title="Service layer">
        {isPending ? (
          ...
          <ErrorState description="The profile service did not respond." onRetry={() => refetch()} />
        ) : (
      ...
      <Section title="Inputs">
        ...
        <Input label="Test date" placeholder="Select a date" error="Pick a date at least a week away." />
        <TextArea
      ...
      <Section title="Data display">
        <View className="flex-row border-y-2 border-divider">
          ...
          <View className="w-px bg-divider" />
          <StatCell label="Gap" value="1.0" detail="to target" tone="accent" className="flex-1 pl-4" />
        </View>
      ...
      <Section title="Rows">
        <View className="border-y border-divider">
          <ListRow label="Listening" description="Section 3 · 10 questions" icon={Headphones} onPress={() => {}} />
          <Rule />
          <ListRow label="Reading" description="Matching headings" icon={BookOpen} onPress={() => {}} />
          <Rule />
